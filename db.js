...
// --- Функции для работы с картами дня ---
getDailyCard = (userId, date) => {
    return db.prepare(`
      SELECT * FROM daily_cards WHERE userId = ? AND date = ?
    `).get(userId, date);
};

saveDailyCard = (userId, date, cardId) => {
    return db.prepare(`
      INSERT OR REPLACE INTO daily_cards (userId, date, cardId)
      VALUES (?, ?, ?)
    `).run(userId, date, cardId);
};

toggleDailyNotifications = (userId) => {
    const current = db.prepare(`
      SELECT daily_card_notifications FROM users WHERE userId = ?
    `).get(userId);

    if (!current) return null;

    const newValue = current.daily_card_notifications ? 0 : 1;
    db.prepare(`
      UPDATE users SET daily_card_notifications = ? WHERE userId = ?
    `).run(newValue, userId);
...
getUserBirthdate = (userId) => {
    const result = db.prepare("SELECT birthday FROM users WHERE userId = ?").get(userId);
    return result ? result.birthday : null;
};

// Установить дату рождения (добавить или обновить)
setUserBirthdate = (userId, birthday) => {
    db.prepare("UPDATE users SET birthday = ? WHERE userId = ?").run(birthday, userId);
    return true;
};
// Сохранить финальную интерпретацию для карты дня
saveDailyInterpretation = (userId, date, interpretation) => {
    db.prepare(`
      UPDATE daily_cards
      SET interpretation = ?
      WHERE userId = ? AND date = ?
    `).run(interpretation, userId, date);
};
